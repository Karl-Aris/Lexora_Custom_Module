...
class MailMail(models.Model):
    ...
    def _prepare_outgoing_list(self, recipients_follower_status=None):
        original_res = super()._prepare_outgoing_list(recipients_follower_status=recipients_follower_status)
        ...
        base_msg = original_res[0] if original_res else {}
        original_body = base_msg.get("body", "")
        original_headers = base_msg.get("headers", {})

        result = []

        # === Clean message for To + CC (no BCC note) ===
        if to_partners or cc_partners:
            to_cc_msg = base_msg.copy()
            to_cc_msg.update({
                "email_to": email_to,
